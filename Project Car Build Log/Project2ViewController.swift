...
class Project2ViewController: UIViewController {
    ...
    @IBOutlet weak var P2BannerView: GADBannerView!
    ...
    override func viewDidLoad() {
        ...
        setupConstraints()
        PartInfo = general.readFile(fileName: "file2.txt")
        ...
        setup()
        if general.getPro() {
            P2BannerView.isHidden = true
        } else {
            self.P2BannerView.adUnitID = general.getGADUnitID(storyboard: "Project")
            self.P2BannerView.rootViewController = self
            P2BannerView.load(GADRequest())
        }
    }
    ...
    func setupConstraints() {
        ...
        if general.getPro() {
            ...
            P2BannerView.center.x = centerX
            P2BannerView.center.y = view.frame.maxY - 50
            Version.center.y = P2BannerView.center.y - 35
        }
        ...
        self.view.bringSubviewToFront(P2BannerView)
    ...
    @IBAction func AddPartPushed(_ sender: UITapGestureRecognizer) {
        if (!Part1Exists) {
    ...
    func writeNewStuff(partNum: Int, partName: String, partCost: String, partDate: String, doIExist: String, edit: Bool) {
        ...
        if (doIExist == "true") {
            if (!edit) {
                //only adds to part count if user not editing part
        ...
        general.reorganizeParts(project: 2, partRemoved: partNum)
        setupConstraints()
        setup()
    }
